...
void copy_lpo_letter(LPOLetter_T *_new, LPOLetter_T *old,
		     LPOLetterRef_T old_to_new[],
		     int iseq_new[])
{
  LPOLetterLink_T *link;
  _new->letter=old->letter; /* SAVE ITS SEQUENCE LETTER */
  add_lpo_sources(&_new->source,&old->source,iseq_new); /* SAVE SOURCES */
  for (link= &old->left;link && link->ipos>=0;link=link->more) /*SAVE left*/
    add_lpo_link(&_new->left,old_to_new[link->ipos]);
  for (link= &old->right;link && link->ipos>=0;link=link->more)/*SAVE right*/
    add_lpo_link(&_new->right,old_to_new[link->ipos]);
  return;
}
