...
int main(int argc,char *argv[])
{
  ...
  char score_file[256],seq_file[256],po_list_entry_filename[256],*comment=NULL,*al_name=(char*)"test align";
  ...
  char *reference_seq_name=(char*)"CONSENS%d",*clustal_out=NULL;

  black_flag_init(argv[0],(char*)PROGRAM_VERSION);
  ...
  if (n_input_seqs == 0) { /* HMM.. NO DATA. */
    ...
    goto free_memory_and_exit;
      ...
      lpo_out = buildup_progressive_lpo (n_input_seqs, input_seqs, &score_matrix,
				       ...
				       matrix_scoring_function, do_global, do_preserve_sequence_order, fp_in, fp_out);
      fclose(fp_in);
      fclose(fp_out);
  }
