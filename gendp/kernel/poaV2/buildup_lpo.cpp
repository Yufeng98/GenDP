...
LPOSequence_T *buildup_progressive_lpo(int nseq,LPOSequence_T **all_seqs,
					...
					ResidueScoreMatrix_T *),
                                       int use_global_alignment,
				       int preserve_sequence_order, FILE* fp_in, FILE* fp_out)
{
  ...
  for (iscore=0;iscore<nscore;iscore++) {
    ...
    buildup_pairwise_lpo(new_seq,all_seqs[cluster_j],score_matrix,
			 use_aggressive_fusion,
                         scoring_function,use_global_alignment, fp_in, fp_out);
                                    ...
                                    int use_global_alignment, FILE* fp_in, FILE* fp_out)
{
  ...
  // align_lpo_po(seq1, seq2, score_matrix, &al1, &al2,  scoring_function, use_global_alignment); /* ALIGN TWO POS */
  // early_break = align_lpo_po_accelerator(seq1, seq2, score_matrix, &al1, &al2,  scoring_function, use_global_alignment); /* ALIGN TWO POS */
  early_break = align_lpo_po_GenDP(seq1, seq2, score_matrix, &al1, &al2,  scoring_function, use_global_alignment, fp_in, fp_out); /* ALIGN TWO POS */
