...
namespace constants_with_precision {
  ...
  template <>
  constexpr double INITIAL_CONSTANT_WITH_PRECISION<double>() {
    // return .000000;
    return std::numeric_limits<double>::max() / 16;
  }

  template <>
  constexpr float INITIAL_CONSTANT_WITH_PRECISION<float>() {
    // return 1329227995784915872903807060280344576.000000;
    return std::numeric_limits<float>::max() / 16;
  }
...
public:
 ...
 protected:
  ...
  Haplotype<PRECISION> pad_and_reverse_haplotype(const Haplotype<uint8_t>& haplotype, const size_t left_padding, const size_t right_padding) const {
    ...
    // fprintf(stderr, "%d %d %d %d\n", left_padding, haplotype.bases.size(), right_padding, p.bases.size());
    return p;
  }
  ...
  Read<PRECISION,PRECISION> pad_read(const Read<uint8_t,uint8_t>& read) const {
    ...
    padded_read.base_qual_char      = pad_and_convert_qual<PRECISION, false>(read.base_qual_char, READ_LEFT_PADDING, READ_RIGHT_PADDING);
  ...
  std::vector<double> calculate (const std::vector<Read<PRECISION,PRECISION>>& padded_reads, const std::vector<Haplotype<PRECISION>>& padded_haplotypes) {
    ...
    for (const auto& read : padded_reads) {
      ...
      for (const auto& hap : padded_haplotypes) {
        // m_diagonals.update(INITIAL_CONSTANT / hap.original_length, m_max_original_read_length + total_read_padding());
        m_diagonals.update(INITIAL_CONSTANT / hap.original_length);
        // for (int r = 0; r < m_max_original_read_length + total_read_padding(); ++r)
        //   printf("%d %f %f %f %f %f %f %f %f %f\n", r, m_diagonals.mpp[r], m_diagonals.xpp[r], m_diagonals.ypp[r], m_diagonals.mp[r], m_diagonals.xp[r], m_diagonals.yp[r], m_diagonals.m[r], m_diagonals.x[r], m_diagonals.y[r]);
        
        // printf("%f\n", m_diagonals.mpp[1]);

        // printf("%d %d %d %d %d %d %d %d %d\n", m_diagonals.mpp.size(), m_diagonals.xpp.size(), m_diagonals.ypp.size(), m_diagonals.mp.size(), m_diagonals.xp.size(), m_diagonals.yp.size(), m_diagonals.m.size(), m_diagonals.x.size(), m_diagonals.y.size());
        // printf("%f %f %f %f %f %f %f %f %f\n", m_diagonals.mpp[0], m_diagonals.xpp[0], m_diagonals.ypp[0], m_diagonals.mp[0], m_diagonals.xp[0], m_diagonals.yp[0], m_diagonals.m[0], m_diagonals.x[0], m_diagonals.y[0]);
  ...
  std::vector<double> calculate_pe (const std::vector<Read<PRECISION,PRECISION>>& padded_reads, const std::vector<Haplotype<PRECISION>>& padded_haplotypes) {
    auto results = std::vector<double>{};
    results.reserve(padded_reads.size() * padded_haplotypes.size());
    for (const auto& read : padded_reads) {
      m_constants.update(read);
  ...
  void calculate_failed (const Testcase& testcase, std::vector<double>& results) {
    auto master_idx = 0;
    auto padded_reads = Read<PRECISION,PRECISION>{};
    for (auto read : testcase.reads) {
      ...
      for (auto hap_idx = 0u; hap_idx != testcase.haplotypes.size(); ++hap_idx) {
        if (results[master_idx] == FAILED_RUN_RESULT) {
          if (!has_padded_read) {
            padded_reads = pad_read(read); // making a copy here so I can keep it across haplotype iterations
            m_constants.update(padded_reads);
          ...
          // m_diagonals.update(INITIAL_CONSTANT/padded_haplotype.original_length, m_max_original_read_length + total_read_padding());
          m_diagonals.update(INITIAL_CONSTANT/padded_haplotype.original_length);
          results[master_idx] = compute_full_prob(padded_reads, padded_haplotype);
        }
        ++master_idx;
      }
    }
